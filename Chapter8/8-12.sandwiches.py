'''
8-12. Sandwiches: Write a function that accepts a list of items a person wants on a sandwich . The function should have one parameter that collects as many items as the function call provides, and it should print a summary of the sand- wich that is being ordered . Call the function three times, using a different num- ber of arguments each time .
'''

def sandwiches(*sandwichToppings):
    
    print(f"\nThe toppings in the sandwich are as follows: ")
    for eachTopping in sandwichToppings:
        print(eachTopping.title())

sandwiches('cheese', 'corn', 'chilly')
sandwiches("chicken", "pepperoni")
sandwiches("mushrooms")